...
export class PackData {
    ...
    public static parse(buffer: Buffer) {
        ...
        try {
            var keySize = buffer.readInt32BE(4);
            var key = buffer.toString("utf8", 4 + 4, 4 + 4 + keySize);

            var descSize = buffer.readInt32BE(4 + 4 + 256);
            var desc = buffer.toString("utf8", 4 + 4 + 256 + 4, 4 + 4 + 256 + 4 + descSize);

            var bufferSize = buffer.readInt32BE(4 + 4 + 256 + 4 + 256);
            var _buffer = Buffer.alloc(bufferSize);
            buffer.copy(_buffer, 0, 4 + 4 + 256 + 4 + 256 + 4, 4 + 4 + 256 + 4 + 256 + 4 + bufferSize);
